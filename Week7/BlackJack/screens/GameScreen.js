...
function GameScreen(props) {
  ...
  //Set state variables for the drwn cards
  ...
  function drawUserCardHandler() {
    ...
    //Calculate the new score for the user to see if the card will make the playerbust
  ...
  function drawComputerCardHandler() {
    ...
    setComputerHand((prevComputerCard) => {
      return [computerCard, ...prevComputerCard];
    });
  ...
  function stayHandler() {
    //Set the user finished to true
    setUserFinished(true);

    //Attempt to add up to 20 cards to the computer hand
    if (computerScore <= 16) {
  ...
  }, [userScore]); //Dependent on userScore and will activate ehn userScore changes
